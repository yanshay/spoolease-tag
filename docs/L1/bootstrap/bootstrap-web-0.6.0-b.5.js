...
(async () => {
  ...
  if (fwdidsuccess) {
    const devHostname = await getDBValue("devHostname");
    if (devHostname) {
      const devProtocol = await getDBValue("devProtocol");
      const devPort = await getDBValue("devPort");
      url.protocol = devProtocol;
      url.hostname = devHostname;
      url.port = devPort;
    }
  }
